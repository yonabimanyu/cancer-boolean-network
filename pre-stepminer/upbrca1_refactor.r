# ===============================================================================
# PROCESS upBRCA1 SAMPLES: TRANSCRIPTOMICS AND EPIGENOMICS
# ===============================================================================
#
# Title: Data Processing Pipeline for upBRCA1 Samples
# Description: Extracts and processes chromosome 17 gene expression and DNA
#              methylation data from samples classified as upBRCA1 (upregulated
...
#
# Output Files:
#   - results/exp_upbrca1-expr.txt: Processed gene expression data (Chr17 only)
#   - results/met_upbrca1-expr.txt: Processed DNA methylation data (Chr17 only)
#
...
cat("=== PROCESSING upBRCA1 GENE EXPRESSION DATA ===\n")

# -------------------------------------------------------------------------------
# 1.1 Extract upBRCA1 Samples
# -------------------------------------------------------------------------------
...
# effects on chromosome 17 genes
# -------------------------------------------------------------------------------

cat("1. Extracting upBRCA1 samples from metadata...\n")

upbrca1_sample_ids <- rownames(meta_combined)[
  meta_combined$BRCA1_status == "upBRCA1" & 
  !is.na(meta_combined$BRCA1_status)
]

cat("   Found", length(upbrca1_sample_ids), "upBRCA1 samples\n")

# Subset expression matrix to upBRCA1 samples only
exp_matrix_upbrca1 <- exp_combined[, upbrca1_sample_ids]
...
gene_variance_upbrca1 <- apply(exp_matrix_upbrca1, 1, var)
exp_matrix_upbrca1 <- exp_matrix_upbrca1[gene_variance_upbrca1 > 0, ]

cat("   Genes with variance > 0:", sum(gene_variance_upbrca1 > 0), "\n")
cat("   Matrix dimensions:", dim(exp_matrix_upbrca1)[1], "genes x", 
    dim(exp_matrix_upbrca1)[2], "samples\n")
...
missing_ensembl <- sum(is.na(gene_annotation_upbrca1$ensembl_id) | 
                       gene_annotation_upbrca1$ensembl_id == "")
missing_symbol <- sum(is.na(gene_annotation_upbrca1$gene_symbol) | 
                      gene_annotation_upbrca1$gene_symbol == "")

cat("   Missing Ensembl IDs:", missing_ensembl, "\n")
cat("   Missing Gene Symbols:", missing_symbol, "\n")

unmapped_genes <- gene_annotation_upbrca1[
  !is.na(gene_annotation_upbrca1$ensembl_id) & 
  gene_annotation_upbrca1$ensembl_id != "" & 
  (is.na(gene_annotation_upbrca1$gene_symbol) | 
   gene_annotation_upbrca1$gene_symbol == ""), 
]
...
gene_annotation_upbrca1 <- gene_annotation_upbrca1[
  !(is.na(gene_annotation_upbrca1$gene_symbol) | 
    gene_annotation_upbrca1$gene_symbol == ""),
]

cat("   Genes with valid symbols:", nrow(gene_annotation_upbrca1), "\n")

# Filter for chromosome 17 only
gene_annotation_chr17_upbrca1 <- gene_annotation_upbrca1[
  !is.na(gene_annotation_upbrca1$chromosome) & 
  gene_annotation_upbrca1$chromosome == "17",
]

cat("   Chromosome 17 genes:", nrow(gene_annotation_chr17_upbrca1), "\n")

# Create data.table with expression values
expr_data_chr17_upbrca1 <- data.table(
  ensembl_id = gene_annotation_chr17_upbrca1$ensembl_id,
  gene_symbol = gene_annotation_chr17_upbrca1$gene_symbol,
  chromosome = gene_annotation_chr17_upbrca1$chromosome,
  row_index = gene_annotation_chr17_upbrca1$row_index,
  exp_matrix_upbrca1[gene_annotation_chr17_upbrca1$row_index, ]
)
...
sample_columns_expr <- names(expr_data_chr17_upbrca1)[
  !names(expr_data_chr17_upbrca1) %in% c("ensembl_id", "gene_symbol", 
                                          "chromosome", "row_index")
]

representative_genes_upbrca1 <- expr_data_chr17_upbrca1[, {
...
cat("   Representative genes selected:", nrow(representative_genes_upbrca1), "\n")
...
setcolorder(representative_genes_upbrca1, 
            c("ensembl_id", "gene_symbol", 
              setdiff(colnames(representative_genes_upbrca1), 
                      c("ensembl_id", "gene_symbol", "chromosome", "row_index"))))

# Remove unnecessary columns
representative_genes_upbrca1[, c("row_index", "chromosome") := NULL]

# Rename columns to standard format
representative_genes_upbrca1 <- representative_genes_upbrca1 %>%
  rename(
    ProbeID = ensembl_id,
    Name = gene_symbol
  )
...
# Export to file
write.table(representative_genes_upbrca1, 
            file = "results/exp_upbrca1-expr.txt", 
            sep = "\t", 
            row.names = FALSE, 
            col.names = TRUE, 
            quote = FALSE)

cat("   Expression data saved to: results/exp_upbrca1-expr.txt\n")
...
cat("=== PROCESSING upBRCA1 DNA METHYLATION DATA ===\n")

# -------------------------------------------------------------------------------
# 2.1 Extract upBRCA1 Samples
# -------------------------------------------------------------------------------
...
cat("1. Extracting upBRCA1 samples from methylation metadata...\n")

upbrca1_met_sample_ids <- rownames(meta_combined_met)[
  meta_combined_met$BRCA1_methylation_status == "upBRCA1" & 
  !is.na(meta_combined_met$BRCA1_methylation_status)
]

cat("   Found", length(upbrca1_met_sample_ids), "upBRCA1 methylation samples\n")

# Subset methylation matrix
met_matrix_upbrca1 <- met_combined[, upbrca1_met_sample_ids]

cat("   Matrix dimensions:", dim(met_matrix_upbrca1)[1], "probes x", 
    dim(met_matrix_upbrca1)[2], "samples\n")
...
probe_ids_upbrca1 <- rownames(met_matrix_upbrca1)

probe_mapping_df <- data.frame(
  probeID = probe_ids_upbrca1,
  row_index = seq_len(nrow(met_matrix_upbrca1)),
  stringsAsFactors = FALSE
)
...
probe_annotation_upbrca1 <- merge(
  probe_mapping_df,
...
probe_annotation_upbrca1 <- probe_annotation_upbrca1[
  !(is.na(probe_annotation_upbrca1$gene_name) | 
    probe_annotation_upbrca1$gene_name == ""), 
]

cat("   Probes with valid gene annotations:", nrow(probe_annotation_upbrca1), "\n")
...
probe_annotation_chr17_upbrca1 <- probe_annotation_upbrca1[
  !is.na(probe_annotation_upbrca1$chromosome_name) & 
  probe_annotation_upbrca1$chromosome_name == "17",
]

cat("   Chromosome 17 probes:", nrow(probe_annotation_chr17_upbrca1), "\n")
...
met_data_chr17_upbrca1 <- data.table(
  probeID = probe_annotation_chr17_upbrca1$probeID,
  gene_name = probe_annotation_chr17_upbrca1$gene_name,
  chromosome_name = probe_annotation_chr17_upbrca1$chromosome_name,
  row_index = probe_annotation_chr17_upbrca1$row_index,
  met_matrix_upbrca1[probe_annotation_chr17_upbrca1$row_index, ]
)
...
sample_columns_met <- names(met_data_chr17_upbrca1)[
  !names(met_data_chr17_upbrca1) %in% c("probeID", "gene_name", 
                                         "chromosome_name", "row_index")
]

representative_probes_upbrca1 <- met_data_chr17_upbrca1[, {
...
cat("   Representative probes selected:", nrow(representative_probes_upbrca1), "\n")
...
# Reorder columns
setcolorder(representative_probes_upbrca1, 
            c("probeID", "gene_name", 
              setdiff(colnames(representative_probes_upbrca1), 
                      c("probeID", "gene_name", "chromosome_name", "row_index"))))

# Standardize column names
setnames(representative_probes_upbrca1, "gene_name", "gene_symbol")

# Remove unnecessary columns
representative_probes_upbrca1[, c("row_index", "chromosome_name") := NULL]

# Rename to standard format
representative_probes_upbrca1 <- representative_probes_upbrca1 %>%
  rename(
    ProbeID = probeID,
    Name = gene_symbol
  )

# Quality check: Verify uniqueness
if (length(unique(representative_probes_upbrca1$ProbeID)) != 
    nrow(representative_probes_upbrca1)) {
  warning("Duplicate ProbeIDs detected!")
}

if (length(unique(representative_probes_upbrca1$Name)) != 
    nrow(representative_probes_upbrca1)) {
  warning("Duplicate gene names detected!")
}

# Export to file
write.table(representative_probes_upbrca1, 
            file = "results/met_upbrca1-expr.txt", 
            sep = "\t", 
            row.names = FALSE, 
            col.names = TRUE, 
            quote = FALSE)

cat("   Methylation data saved to: results/met_upbrca1-expr.txt\n")
