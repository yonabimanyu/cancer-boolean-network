...
sub printAllRelations {
  my($self, $indexFile, $type) = @_;
  my $version = $self->{'major'}.".".$self->{'minor'};
  
  if ($version ne "1.1") {
    print STDERR "Error: Only version 1.1 supported (current: $version)\n";
    exit(1);
  }
  
  # Validate type parameter
  if ($type < 0 || $type > 6) {
    print STDERR "Error: Invalid type $type (must be 0-6)\n";
    exit(1);
  }
  ...
  # Print header
  print "source_ensembl_id\tsource_gene\ttarget_ensembl_id\ttarget_gene\trelation_type\n";
  ...
  for (my $i = 0; $i < $self->{'num'}/2; $i++) {
    ...
    if ($i % 1000 == 0 && $i > 0) {
      print STDERR "Progress: $i/" . ($self->{'num'}/2) . " genes processed\n";
    }
    ...
    for (my $j = 0; $j < $self->{'num'}/2; $j++) {
      ...
      # Filter by type
      next if ($type > 0 && $code != $type);
      next if ($code == 0);
      
      my $probeid_a = $balanced->[$i];
      my $probeid_b = $balanced->[$j];
      
      # CRITICAL: Check if probes exist in index
      my $idx_a_ref = $hashTable->[0]->{uc($probeid_a)};
      my $idx_b_ref = $hashTable->[0]->{uc($probeid_b)};
      
      if (!defined $idx_a_ref || !defined $idx_b_ref) {
        $total_skipped++;
        print STDERR "Warning: Probe not in index: " . 
                     (!defined $idx_a_ref ? $probeid_a : $probeid_b) . "\n"
                     if ($total_skipped <= 10);  # Limit warnings
        next;
      }
      
      my $idx_a = $idx_a_ref->[0];
      my $idx_b = $idx_b_ref->[0];
      
      # Get gene names with proper fallback
      my $name_a = $hashTable->[3]->{$idx_a};
      my $name_b = $hashTable->[3]->{$idx_b};
      
      # Normalize empty/placeholder names (consistent with other methods)
      if (!defined $name_a || $name_a eq "---" || $name_a eq "-" || $name_a eq "") {
        $name_a = "Unknown";
      }
      if (!defined $name_b || $name_b eq "---" || $name_b eq "-" || $name_b eq "") {
        $name_b = "Unknown";
      }
      
      # Relation type mapping
      my @types = ("None", "Low → High", "Low → Low", "High → High", 
                   "High → Low", "Equivalent", "Opposite");
      
      print "$probeid_a\t$name_a\t$probeid_b\t$name_b\t$types[$code]\n";
      $total_found++;
    }
  }
  ...
  print STDERR "\n=== Summary ===\n";
  print STDERR "Total combinations processed: $total_processed\n";
  print STDERR "Relations found: $total_found\n";
  print STDERR "Probes skipped (not in index): $total_skipped\n";
}
...
if ($cmd eq "all") {
  my ($file, $indexFile, $type) = @ARGV;
  my $n = Network->new(file => $file, mode => "r");
  $n->readNetworkFile();
  $n->printAllRelations($indexFile, $type);
  $n->close();
}
