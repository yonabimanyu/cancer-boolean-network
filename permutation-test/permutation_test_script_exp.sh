...
# ENHANCED BooleanNet Permutation Test Framework
...
STAT_THRESHOLDS=(1.5 1.6 1.7 1.9)
...
validate_environment() {
    ...
    # Validate BV file format
    local header_line=$(head -n 1 "$INPUT_BV")
    if [[ ! "$header_line" =~ ensembl_id.*gene_symbol.*BitVector ]]; then
        log_error "BV file header format is incorrect. Expected: ensembl_id<tab>gene_symbol<tab>BitVector"
        exit 1
    fi
    
    # Count genes in BV file
    local gene_count=$(tail -n +2 "$INPUT_BV" | wc -l)
    log_info "📊 Found $gene_count genes in BitVector file"
    
    # Extract sample genes from index file for validation
    local sample_genes=$(tail -n +4 "$INPUT_IDX" | awk '{print $1}' | grep -E '^ENSG[0-9]+$' | head -5)
    if [ -z "$sample_genes" ]; then
        log_error "No valid Ensembl Gene IDs found in index file"
        exit 1
    fi
...
create_permuted_bitvector() {
    ...
    # Python script for row-wise BitVector permutation
    python3 -c "
import sys
import pandas as pd
import numpy as np
...
try:
    ...
    required_cols = ['ensembl_id', 'gene_symbol', 'BitVector']
...
PURPOSE:
Find optimal statistical threshold for Boolean implication analysis
through permutation testing and False Discovery Rate (FDR) control.

INPUT FILES:
  - BitVector File: $INPUT_BV
...
PARAMETERS:
  ...
  - Processing: ALL genes (no sampling)
  - Total Analyses: $((${#STAT_THRESHOLDS[@]} * (N_PERMUTATIONS + 1)))

METHODOLOGY:
1. Original Analysis: Run BooleanNet on original data for each threshold
2. Permutation Test: Randomly permute BitVectors and re-analyze
...
main() {
    ...
    echo -e "${MAGENTA}================================================================================${NC}"
    echo -e "${MAGENTA}🧬 BooleanNet Permutation Test Framework v8.0${NC}"
    echo -e "${MAGENTA}================================================================================${NC}"
    
    log_info "🚀 Starting Boolean implication analysis with permutation testing..."
    log_info "📊 Statistical thresholds to test: ${STAT_THRESHOLDS[*]}"
    log_info "🎲 Permutations per threshold: $N_PERMUTATIONS"
    log_info "🧬 Processing: ALL genes (no sampling)"
    ...
    echo -e "${MAGENTA}================================================================================${NC}"
    echo -e "${GREEN}🎊 BooleanNet Permutation Test completed successfully!${NC}"
}
